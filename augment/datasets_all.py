...
from augment.arguments import DataTrainingArguments
from augment.input_example import InputFeatures, EntityType, RelationType, Entity, Relation, Intent, InputExample, CorefDocument
from augment.base_dataset import BaseDataset
from augment.utils import get_precision_recall_f1
from augment.coreference_metrics import CorefAllMetrics
from augment.input_formats import INPUT_FORMATS
from augment.output_formats import OUTPUT_FORMATS
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        with open(file_path, 'r') as f:
            for i, line in enumerate(f):
                line = line.split('|||')
                line = line[0:-1]  # avoid errors in preprocessing
                assert len(line) == 2
